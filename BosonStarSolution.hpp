...
private: // private member variables/arrays
    ...
    double PSC=1., OMC=0.5; // central density of scalar field (0.272 for kaup)  PSC and OMC are central values of conformal factor and lapse, not important as long as they are sensible (i.e. order 1)
    ...
    const int adaptive_stepsize_repetitions = 20;//50; // 0 for no adaptive
  	double L, dx, WW, ww = 2.; // L, length of domain, dx.
...
private: // private member fucntions functions
    ...
    double small_P_RHS(const double x, const double P, const double DP, const double PSI, const double OM, const double ww_);
    double P_RHS(const double x, const double P, const double DP, const double PSI, const double OM, const double ww_);
    double DP_RHS(const double x, const double P, const double DP, const double PSI, const double OM, const double ww_);
    double PSI_RHS(const double x, const double P, const double DP, const double PSI, const double OM, const double ww_);
    double OMEGA_RHS(const double x, const double P, const double DP, const double PSI, const double OM, const double ww_);
    void initialise();
    int crossings();
    void fix();
...
public:
    BosonStarSolution();
    void set_initialcondition_params(BosonStarParams &m_params_BosonStar, const double max_r);
